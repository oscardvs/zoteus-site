---
title: Self-host Zoteus as a claude.ai or ChatGPT connector
description: "Add Zoteus as a claude.ai custom connector or a ChatGPT plugin: run your own OAuth 2.1 + PKCE authorization server in front of the Streamable HTTP /mcp endpoint."
---

claude.ai and ChatGPT connect to remote MCP servers **from the cloud**, so a connector must be reachable at a **public HTTPS URL** and protected by **OAuth 2.1 + PKCE** (claude.ai's connector UI has no field for a static API key or `Authorization` header). Since v0.9.0, Zoteus can be that connector: it runs its own OAuth 2.1 authorization server in front of the Streamable HTTP `/mcp` endpoint.
...
- claude.ai and ChatGPT self-register via **Dynamic Client Registration** (RFC 7591), run the **authorization-code + PKCE (S256)** flow, and exchange the code for a short-lived **opaque bearer token**.
...
ChatGPT adds a remote MCP server as a plugin, behind its **Developer mode** switch (**Settings**, then **Security and login**; OpenAI offers it on the paid ChatGPT plans, in the web app). It connects from OpenAI's cloud, so the URL must be the public HTTPS one, never `localhost`.

1. Open **Plugins** (left sidebar, or [chatgpt.com/plugins](https://chatgpt.com/plugins)) and click **Create app**.
2. In the **New Plugin** dialog: Name `Zoteus`, Connection **Server URL**, MCP Server URL `https://<your-host>/mcp`, Authentication **OAuth**. Leave **Advanced OAuth settings** empty: ChatGPT discovers the endpoints from the metadata above and registers itself via DCR (it may show *CIMD unavailable*, which is expected). Tick the acknowledgement and click **Create**.
3. Click **Sign in with Zoteus**. The popup opens the Zoteus consent page: enter the `ZOTEUS_OAUTH_PASSCODE` in passcode mode, or sign in at zotero.org in `zotero` mode, and authorize.
4. If the plugin page says *No app actions available yet*, click **Refresh** in its **Information** section; the tool list then loads.

[Connect ChatGPT to Zotero](/docs/connect-chatgpt-to-zotero) walks through the same dialog for the hosted connector, with the permission prompts and the troubleshooting table.
...
- **Dynamic Client Registration.** Claude registers a fresh public client per connection; ChatGPT registers a confidential client with a client secret. Zoteus caps the in-memory client store (FIFO) and sweeps expired state.
