...
For normal setup, start with [search readiness](/docs/search-readiness). The full [repository configuration reference](https://github.com/oscardvs/zoteus/blob/main/docs/configuration.md) covers additional tuning options.
...
| `ZOTEUS_TRANSLATION_SERVER_URL` | `http://127.0.0.1:1969` | Add-by-identifier/URL (available). |
| `ZOTEUS_EMBEDDINGS` | `local` | Requested embedding provider: local, openai, gemini or off. Local needs an extra dependency; inspect active status. |
| `ZOTEUS_SCHOLAR_PROVIDERS` | `openalex` | Comma list of scholarly-graph providers (available). |
| `ZOTEUS_TRANSFORMERS_PATH` | unset | Absolute folder containing the optional local embedding dependency, usually its `node_modules` directory. |
| `ZOTEUS_INDEX_FULLTEXT` | `false` | Opt in to indexing available extracted PDF body text. |
| `ZOTEUS_INDEX_FULLTEXT_MAX_CHARS` | `40000` | Indexed body-text characters per item; 0 removes this cap. |
| `ZOTEUS_INDEX_MAX_ITEMS` | `5000` | Top-level items per full index build; a tool limit can only lower it. |
| `ZOTEUS_INDEX_OWN_WORDS` | `true` | Include child notes and PDF annotations in indexed search. |
| `ZOTEUS_USAGE_LOG` | `false` | Optional operator-local tool outcomes without argument values or query text. |
| `ZOTEUS_DATA_DIR` | OS data dir | Index + caches location. |
| `ZOTEUS_CONTACT_EMAIL` |: | Polite-pool contact for external scholarly APIs. |
| `ZOTEUS_ALLOW_DELETE` | `false` | Must be `true` to expose permanent delete (available). |
...
## Remote OAuth (claude.ai or ChatGPT remote connector)

Turn the Streamable HTTP `/mcp` endpoint into an OAuth 2.1 + PKCE protected resource so it can be added as a claude.ai custom connector or a ChatGPT plugin. See [Remote OAuth](/docs/remote-oauth) for the full walkthrough.
