---
title: "Zotero research prompts: literature review, synthesis and screening"
...
Use the [first-research-task workflow](/docs/first-research-task) with any deployed version. The current source prompts require passage retrieval before detailed synthesis and distinguish an existing reference from support for a claim. Collection overviews based on abstracts must say so. These revised templates take effect when that source version is installed; hosted releases may lag.

If your client does not expose MCP prompts, paste the workflow from the guide into chat. [Annotations to literature notes](/docs/annotations-to-literature-notes) uses the existing tools and asks you to review a draft before saving.
