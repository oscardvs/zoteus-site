...
| ChatGPT asks permission on every write | The plugin's **Permissions** setting is on **Always ask** or **Allow read actions**. Set it back to **Allow low-risk actions** (the default) or to **Allow all actions** on the plugin page under **Plugins**, or choose **Always allow** when it asks. |
